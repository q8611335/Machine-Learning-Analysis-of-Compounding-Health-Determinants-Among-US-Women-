...
# Tune C (regularisation strength), equivalent to learning rate:
# Small C = stronger regularisation = more conservative model
# Large C = weaker regularisation  = more flexible model
...
for C in [0.001, 0.01, 0.1, 1.0, 10.0]:
    ...
    print(f"  C={C:<8} → val balanced_accuracy = {val_score:.4f}")

    if val_score > best_val_score:
        best_val_score = val_score
        best_C = C

print(f"\nBest C: {best_C}  (val balanced_accuracy = {best_val_score:.4f})")
