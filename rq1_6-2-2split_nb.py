...
# Tune var_smoothing: controls how much variance is added to stabilise
# the Gaussian likelihood estimate (larger = more smoothing/regularisation)

print("\n── Hyperparameter search (validation set) ──")
best_smoothing = None
best_val_score = -1

for var_smoothing in [1e-11, 1e-9, 1e-7, 1e-5, 1e-3, 1e-1]:
    ...
    print(f"  var_smoothing={var_smoothing:.0e} → val balanced_accuracy = {val_score:.4f}")

    if val_score > best_val_score:
        best_val_score = val_score
        best_smoothing = var_smoothing

print(f"\nBest var_smoothing: {best_smoothing:.0e}  (val balanced_accuracy = {best_val_score:.4f})")

# ── 7. Train final model with best hyperparameters ───────────────────────────
final_model = Pipeline(steps=[
    ("preprocessor", preprocessor),
    ("classifier", GaussianNB(var_smoothing=best_smoothing)),
])
